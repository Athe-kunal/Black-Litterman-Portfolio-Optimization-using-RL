...
register_env(
    ...
    lambda config: BlackLittermanEnv(
        data_df=train_data, if_confidence=args.if_confidence
    ),
)

train_env_instance = BlackLittermanEnv(train_data, if_confidence=args.if_confidence)
...
for runs in range(5):
    test_env_instance = BlackLittermanEnv(test_data, if_confidence=args.if_confidence)
    ...
    run.log({f"TRANS_Log_data_{runs}_{args.if_confidence}": log_table})
