...
rut2000 = yf.download(tickers=["^RUT"], start=config_params.start_date, end=config_params.end_date, interval="1mo")
...
rut2000.reset_index(inplace=True)

rut2000["ticker"] = "^RUT"
bonds["ticker"] = "AGG"

data = pd.concat([bonds, rut2000], axis=0)
...
def run_mlp_bl_rusell(if_confidence,test_data=test_data):
    ...
    drl_agent = DRLlibv2(
        ...
        num_workers=config_params.num_workers,
        log_level="DEBUG",
        run_name="FINRL_TEST_MLP_RUSELL",
        storage_path="FINRL_TEST_MLP_RUSELL",
    ...
    results_df.to_csv(f"MLP_{if_confidence}_RUSELL.csv")
    ds = []
    ...
    for runs in range(5):
        ...
        run.log({f"MLP_Log_data_{runs}_{if_confidence}_RUSELL": log_table})
